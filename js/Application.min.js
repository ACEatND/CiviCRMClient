...
$(function(){
	...
	function(data){
		if(data.indexOf("Global Configuration") !== -1) {
...
$("#HomePage").live("pagecreate", function () {
	...
	var programs = [{value: 'STT', label: 'ACE Teaching Fellows'}, {value: 'RLP', label: 'RLP'}, {value: 'TEC', label: 'TEC'}, {value: 'ENL', label: 'ENL'}];

        for (index = 0; index < programs.length; index++) {
            optionLabel = programs[index].label;
            optionValue = programs[index].value;

            label = $("<label for='ACEProgram" + index + "Radio'/>");
            label.text(optionLabel);
            $('#IndividualsListPage .ace_options').append(label);
            option = $("<input type='radio' id='ACEProgram" + index + "Radio' class='custom' value='" + index + "' name='ACEProgramRadio' />");
            $('#IndividualsListPage .ace_options').append(option);
            option.checkboxradio();
            option.change(function () {
                cohortInfo = $('#ACESearchString').val();
		console.log(cohortInfo);
                if ($(this).is(":checked") && cohortInfo != null && cohortInfo > 0) {
                    setTimeout('CiviCRMSearchIndividuals(true);', 10);
                }
            });
        }
	customValue = $("<input type='number' id='ACESearchString'/>");
        customValue.change(function () {
            isByAddress = $('#IndividualsListPage .ace_options .custom').is(":checked");
            address = $('#ACESearchString').val();
            if (isByAddress == true && address != null & address.length > 0) {
                setTimeout('CiviCRMSearchIndividuals(true);', 10);
            }
        });
	$('#IndividualsListPage .ace_options').append(customValue);
...
function LoginUser(a)
{
	...
	//data: {username:c,passwd:b,autologin=1});
    $.get('/administrator/index.php', 
        {
            catholicschools: 1, 
            username: c, 
            ...
            if(data.indexOf("Global Configuration") !== -1)
            {
                // console.log(data.indexOf("Global Configuration") !== -1);
                $.mobile.hidePageLoadingMsg();
                $.mobile.changePage("#HomePage");
            } else {
                $.mobile.hidePageLoadingMsg();
    ...
    //            },
    //            {
    //                ajaxURL: "/administrator/index.php",
                                       ...
                                       if (typeof value == 'object') {
                                           ...
                                           d = $("<a/>", {
                                               ...
                                               click: function () {
                                                   ...
                                                   $.mobile.changePage("#EditNoteDetailsPage");
                                                   $.mobile.pageLoading();
                       ...
                       if (result)
                       {
                           if (result.is_error == 0)
                           {
                               if (result.values != undefined && result.values != null)
                               {
                                   ...
                                   $("#NoteSubjectH1").val(subject).addClass('ui-input-text ui-body-null ui-corner-all ui-shadow-inset ui-body-b');
...
function gps_distance_mi(lat1, lon1, lat2, lon2)
{
    ...
    var d = R * c;
//$.mobile.hidePageLoadingMsg(); //added
...
function yahooPlaceFinderGeocode(address, settings) {
    ...
    var geoAPI = 'http://maps.googleapis.com/maps/api/geocode/json?address=' + escape(address) + '&sensor=true'
